...
//foreach ($db_arr as $db_name) {
    ...
    foreach ($uids as $item) {
       ...
       // echo $query . PHP_EOL;
       // $db->insert($query);
        $query = "DELETE FROM `user_score_log` WHERE `time` <='{$bak_time}' AND uid={$uid} AND type not in (11,12)";
        echo $query . PHP_EOL;
        $db->delete($query);
        $number++;
        if ($number>10) {
    ...
    echo $db_name . ' ：user_score_log备份完成';
//}
echo '执行成功';
