...
$db_name = 'shangshui9604';
    echo 'db_name--' . $db_name . PHP_EOL;
    $db_host = 'localhost:2759';
    $db_user = 'root';
    $db_pwd = 'kCYDi3Q1cdKWDkQn';   
    ...
    $query = "DELETE FROM `user_score_log` WHERE `time`  <='{$mktime}' ";
    $db->delete($query);
    ...
    $db->delete($query);
    $query = "DELETE FROM `integral_log` WHERE integral >0 AND `mktime` <='{$mktime}'";
    $db->delete($query);
    
    echo '删除user_score_log数据' . PHP_EOL;
    $query = "DELETE FROM `user_score_log` WHERE `time` <='{$mktime_seven}' and type not in (11,12)";
    echo $query . PHP_EOL;
    $db->delete($query);
