...
class LanTian154
{
    ...
    const LT_GROUPID = 160; //蓝天groupid
    const DF_GROUPID = 97; //对方groupid
    const TOKEN = '4262b4d732e6d6a05d1f0b1805627d507b5e';//蓝天登录token
    const USERNAME = 'zidong02'; //蓝天登录账号
    ...
    public function getCardGameStatus()
    {
        ...
        if (!empty($cardGame)) {
            ...
            //判断上一靴是否清空路单
//            if ($ju_103 == 1) {
//                $clean_boots_number = $boots_number_103 - 1;
//                $iscleanTemp = file_get_contents(self::HTTPURL . '/v1/game154/clean?room_id=' . $curRoomId . '&boots_number=' . $clean_boots_number);
//                $isclean = json_decode($iscleanTemp, true);
//                if ($isclean['code'] == 500) {
//                    $ludanArr = ["cmd" => 4210, "set" => 102, "boots_number" => $isclean['data']['boots_number'] + 1, "ju" => 1, "groupid" => $curRoomId];
//                    $client->text(json_encode($wsArr));
//                    $client->text(json_encode($ludanArr));
//                    $client->close();
//                    $logs = $date . '|' . $paiju . '|靴号+1，清空路单(开局清空)';
//                    echo $logs;
...
//            }
            ...
            $curGameInfoTemp = file_get_contents(self::HTTPURL . '/v1/game154/info?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&game_id=' . $game_id);
            ...
            if ($curGameInfo['lastGame']['lastState'] < 2) {
                if ($curGameInfo['lastGame']['lastState'] == 1) {//上一局处于停止下注状态，先获取上局信息，结算
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $last_ju . '&wsStatus=4&game_id=' . $game_id);
            ...
            if ($start_103 == 1 or $cardGame_last['start'] == 1) {
                if ($curStart == 0 && $curGameInfo['lastGame']['start'] == 0 && !empty($curGameInfo['data'])) {
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=6&game_id=' . $game_id);
            ...
            if (!empty($textData['cmd']) && !empty($textData['set']) && $textData['cmd'] == 4210 && $textData['set'] == 7 && empty($curGameInfo['data'])) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=7&game_id=' . $game_id);
            ...
            if ($curGameState == 0 && time() - $mktime > 20 && $curWsStatus == 1) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=2&game_id=' . $game_id);
            ...
            if ($cardGame103['state'] > $curGameState) {
                if ($cardGame103['state'] == 0) {//开始投注
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=1&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=3&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=4&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game154/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=5&game_id=' . $game_id);
...
//        }
    ...
    public function countAgain()
    {
        ...
        if (!empty($syslog)) {
            ...
            //获取重新结算状态
            $gameAgainRes = file_get_contents(self::HTTPURL . '/v1/game154/gameagain?boots_number=' . $boots_number . '&ju=' . $ju);
            ...
            if ($gameAgainArr['again_status'] == 0) {
                ...
                if (abs($game_result01 - $game_result02) > 0) {
                    ...
                    $client->close();
                    //修改重新结算状态
                    file_get_contents(self::HTTPURL . '/v1/game154/gameagainstatus?card_game_id=' . $gameAgainArr['id']);
                } else {
    ...
    public function docmd()
    {
        ...
        if ($doIt) {
            ...
            $connection = ssh2_connect('122.9.142.204', 1998);
    ...
    {
        $head = file_get_contents('http://agent9605.fmfcys.org/v1/game154/getAiImg');
