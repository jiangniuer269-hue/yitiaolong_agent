...
class LhTongjiManage
{
    ...
    {
        $card_game = LhCardGame::where('bets_merge_state', 0)->where('state', 2)->field('id,room_id,boots_number,ju,zhuang,zhuang_dui,xian_dui,lucky_six,groupid,mktime')->select();
        if (count($card_game) > 0) {
            foreach ($card_game as $item_card_game) {
                ...
                $LhBetsLog = LhBetsLog::field('odds,type,uid,xm_type,xm_rate,agents_share_rate,agents_id,score_before,ip')
                    ->where('card_game_id', $card_game_id)->where('user_ai', 0)->where('tourist', 0)->where('state', 1)->select();
                if (count($LhBetsLog) > 0) {
                    foreach ($LhBetsLog as $bets_log_item) {
                    ...
                    foreach ($betsData as $bets_data_item) {
                        ...
                        if ($zhuang_win == 3) {
                            $user_sb_losewin += $bets_data_item['type3'] * 8;
                        } else {
                            $user_sb_losewin += -$bets_data_item['type3'];
                        }
                        if ($zhuang_dui > 0) {
                            $user_sb_losewin += $bets_data_item['type4'] * 11;
                        } else {
                            $user_sb_losewin += -$bets_data_item['type4'];
                        }
                        if ($xian_dui > 0) {
                            $user_sb_losewin += $bets_data_item['type5'] * 11;
                        } else {
                        ...
                        if ($lucky_six == 0) {
                            ...
                            $user_lucky_losewin += $bets_data_item['type7'] * 12;
                        } elseif ($lucky_six == 7) {
                            $user_lucky_losewin += $bets_data_item['type7'] * 20;
                        }
                        ...
                        if ($user_info['ai'] == 0 && $user_info['tourist'] == 0) {
                            $bets_ukey = $card_game_id . '_' . $uid . '_' . $groupid . '_lh';
                        ...
                        $inte_ukey = $card_game_id . '_1_' . $uid . '_' . $groupid;
                ...
                LhCardGame::where('id', $card_game_id)->update(['bets_merge_state' => 1]);
    ...
    function agents_odds($agents_id, $value, $game_data, $level, $mktime)
    {
        ...
        $insert_game_data['ukey'] = $card_game_id . '_' . $value['uid'] . '_' . $agents_id . '_lh';
        $insert_game_data['game_type'] = 1;
    ...
    function agent_share($agents_zx_losewin, $agents_sb_losewin, $agents_lucky_losewin, $agents_id, $level, $card_game_id, $fan_agents_id, $mktime)
    {
        ...
        $agents_share_data = [
            ...
            'mktime' => $mktime,
            'ukey' => $card_game_id . '_' . $fan_agents_id . '_' . $agents_id . '_lh',
            'game_type' => 1
        ];
