...
class LanTian9701
{
    const  DF_TID = 606;//对方tid
    const LT_TID = 634;//蓝天tid
    //http://agent9602.zmind.xyz  http://www.lantianws.com:90 http://test_agent9602.lwredyv.cn/
    const HTTPURL = 'http://agent9701.shanghaimetal.org';//蓝天后台地址
    const WSURL = 'ws://116.63.133.208:9701';//蓝天WS
    const LT_GROUPID = 167; //蓝天groupid
    const DF_GROUPID = 144; //对方groupid
    const TOKEN = '805696e79218965eb72c92a549dd5a330112';//蓝天登录token
    const USERNAME = 'xiaochun001'; //蓝天登录账号
    ...
    public function getCardGameStatus()
    {
        ...
        if (!empty($cardGame)) {
            ...
            $curGameInfoTemp = file_get_contents(self::HTTPURL . '/v1/game9701/info?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&game_id=' . $game_id);
            ...
            if ($curGameInfo['lastGame']['lastState'] < 2) {
                if ($curGameInfo['lastGame']['lastState'] == 1) {//上一局处于停止下注状态，先获取上局信息，结算
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $last_ju . '&wsStatus=4&game_id=' . $game_id);
            ...
            if (!empty($textData['cmd']) && !empty($textData['set']) && $textData['cmd'] == 4210 && $textData['set'] == 7 && empty($curGameInfo['data'])) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=7&game_id=' . $game_id);
            ...
            if ($start_103 == 1 or $cardGame_last['start'] == 1) {
                if ($curStart == 0 && $curGameInfo['lastGame']['start'] == 0 && !empty($curGameInfo['data'])) {
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=6&game_id=' . $game_id);
            ...
            if ($curGameState == 0 && time() - $mktime > 20 && $curWsStatus == 1) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=2&game_id=' . $game_id);
            ...
            if ($cardGame103['state'] > $curGameState) {
                if ($cardGame103['state'] == 0) {//开始投注
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=1&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=3&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=4&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=5&game_id=' . $game_id);
...
//        }
    ...
    public function countAgain()
    {
        ...
        if (!empty($syslog)) {
            ...
            //获取重新结算状态
            $gameAgainRes = file_get_contents(self::HTTPURL . '/v1/game9701/gameagain?boots_number=' . $boots_number . '&ju=' . $ju);
            ...
            if ($gameAgainArr['again_status'] == 0) {
                ...
                if (abs($game_result01 - $game_result02) > 0) {
                    ...
                    $client->close();
                    //修改重新结算状态
                    file_get_contents(self::HTTPURL . '/v1/game9701/game9701againstatus?card_game_id=' . $gameAgainArr['id']);
                } else {
    ...
    public function docmd()
    {
        ...
        if ($cmdtype == 1) {//开启同步
            $doIt = TRUE;
            $msg = '自动开奖已开启(156)';
            $cmd = "cd /home/www/wwwroot/php-task-156/ && nohup php run.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 2) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(156)';
            $cmd = "cd /home/www/wwwroot/php-task-156/ &&  php run.php stop";//命令
        } elseif ($cmdtype == 3) {
            $doIt = TRUE;
            $msg = '自动开奖已开启(154)';
            $cmd = "cd /home/www/wwwroot/php-task-154/ && nohup php run.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 4) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(154)';
            $cmd = "cd /home/www/wwwroot/php-task-154/ &&  php run.php stop";//命令
        } elseif ($cmdtype == 5) {
            $doIt = TRUE;
            $msg = '自动开奖已开启(155)';
            $cmd = "cd /home/www/wwwroot/php-task-155/ && nohup php run.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 6) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(155)';
            $cmd = "cd /home/www/wwwroot/php-task-155/ &&  php run.php stop";//命令
        }
    ...
    {
        $room = TeamRoom::where('tid', 584)->whereIn('id', [154, 155, 156])->field('groupname,id')->select();
    ...
    {
        $head = file_get_contents('http://agent9605.fmfcys.org/v1/game9701/getAiImg');
    ...
    public function clean()
    {
        ...
        $room_id = $request->get('room_id');
        $boots_number = $request->get('boots_number');
        $game = CardGame::where('room_id', $room_id)->where('boots_number', $boots_number)->where('mktime', '>', time() - 3600)->field('id')->find();
        if (!empty($game)) {
            $game01 = CardGame::where('room_id', $room_id)->where('boots_number', $boots_number)->where('start', 1)->where('mktime', '>', time() - 3600)->field('id')->find();
            if (!empty($game01)) {
                return ['code' => 200, 'msg' => '路单已清空', 'data' => ['boots_number' => $boots_number]];
            } else {
                return ['code' => 500, 'msg' => '上局路单未清空', 'data' => ['boots_number' => $boots_number]];
            }
        } else {
            return ['code' => 200, 'msg' => '靴号不存在', 'data' => ['boots_number' => $boots_number]];
        }
    }
