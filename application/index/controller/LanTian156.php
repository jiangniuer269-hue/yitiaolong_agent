...
class LanTian156
{
    ...
    const LT_GROUPID = 156; //蓝天groupid
    const DF_GROUPID = 99; //对方groupid
    const TOKEN = '1344b4d732e6d6a05d1f0b1805627d507b5e';//蓝天登录token
    const USERNAME = 'zidong01'; //蓝天登录账号
    ...
    public function getCardGameStatus()
    {
        ...
        if (!empty($cardGame)) {
            ...
            $curGameInfoTemp = file_get_contents(self::HTTPURL . '/v1/game156/info?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&game_id=' . $game_id);
            ...
            if ($curGameInfo['lastGame']['lastState'] < 2) {
                if ($curGameInfo['lastGame']['lastState'] == 1) {//上一局处于停止下注状态，先获取上局信息，结算
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $last_ju . '&wsStatus=4&game_id=' . $game_id);
            ...
            if ($start_103 == 1 or $cardGame_last['start'] == 1) {
                if ($curStart == 0 && $curGameInfo['lastGame']['start'] == 0 && !empty($curGameInfo['data'])) {
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=6&game_id=' . $game_id);
            ...
            if (!empty($textData['cmd']) && !empty($textData['set']) && $textData['cmd'] == 4210 && $textData['set'] == 7 && empty($curGameInfo['data'])) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=7&game_id=' . $game_id);
            ...
            if (!empty($curGameInfo['data'])) {
                ...
                $logs = $date . '|' . $paiju . '|对方处于停止下注或已结算状态，牌局状态不匹配，请从下局开始同步';
                echo $logs;
                common::write_log($logs);
                exit();
            }
            ...
            if ($curGameState == 0 && time() - $mktime > 20 && $curWsStatus == 1) {
                ...
                $client->close();
                //修改当局ws状态
                file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=2&game_id=' . $game_id);
            ...
            if ($cardGame103['state'] > $curGameState) {
                if ($cardGame103['state'] == 0) {//开始投注
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=1&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=3&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=4&game_id=' . $game_id);
                    ...
                    $client->close();
                    //修改当局ws状态
                    file_get_contents(self::HTTPURL . '/v1/game156/wsstatus?room_id=' . $curRoomId . '&boots_number=' . $boots_number_103 . '&ju=' . $ju_103 . '&wsStatus=5&game_id=' . $game_id);
...
//        }
    ...
    public function countAgain()
    {
        ...
        if (!empty($syslog)) {
            ...
            //获取重新结算状态
            $gameAgainRes = file_get_contents(self::HTTPURL . '/v1/game156/gameagain?boots_number=' . $boots_number . '&ju=' . $ju);
            ...
            if ($gameAgainArr['again_status'] == 0) {
                ...
                if (abs($game_result01 - $game_result02) > 0) {
                    ...
                    $client->close();
                    //修改重新结算状态
                    file_get_contents(self::HTTPURL . '/v1/game156/game156againstatus?card_game_id=' . $gameAgainArr['id']);
                } else {
    ...
    public function updateRoomInfo()
    {
        ...
        $video_link_web = $request->post('video_link_web');
        $groupname = $request->post('groupname');
        TeamRoom::where('tid', self::LT_TID)->where('groupid', $groupid)->update(['mark' => $mark, 'video_link' => $video_link,'groupname' => $groupname]);
        $updateRoomArr = ["cmd" => 4202, "groupid" => $groupid];
        $client = new Client(self::WSURL);
        $client->text(json_encode(self::loginWs()));
        $client->text(json_encode($updateRoomArr));
        // echo $client->receive();
        $client->close();
        return 1;
    }
    ...
    public function docmd()
    {
        ...
        if ($cmdtype == 1) {//开启同步
            $doIt = TRUE;
            $msg = '自动开奖已开启(156)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-156/ && nohup php run156.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 2) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(156)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-156/ &&  php run156.php stop";//命令
        } elseif ($cmdtype == 3) {
            $doIt = TRUE;
            $msg = '自动开奖已开启(154)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-154/ && nohup php run154.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 4) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(154)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-154/ &&  php run154.php stop";//命令
        } elseif ($cmdtype == 5) {
            $doIt = TRUE;
            $msg = '自动开奖已开启(155)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-155/ && nohup php run155.php start --log=false > /dev/null &";//命令
        } elseif ($cmdtype == 6) {
            $doIt = TRUE;
            $msg = '自动开奖已停止(155)';
            $cmd = "cd /www/wwwroot/agent_shangshui/php-task-155/ &&  php run155.php stop";//命令
        }

        if ($doIt) {
            $user = "root";//远程用户名
            $pass = "EA8r88n!@518*518";//远程密码
            $connection = ssh2_connect('154.23.221.76', 1998);
            ssh2_auth_password($connection, $user, $pass);
            $ret = ssh2_exec($connection, $cmd);
            stream_set_blocking($ret, true);
            return ['code' => 200, 'msg' => $msg,'cmd'=>$cmd];
        } else {
            return ['code' => 500, 'msg' => '参数错误，操作失败','cmd'=>$cmd];
        }
    }
    ...
    {
        $room = TeamRoom::where('tid', 584)->whereIn('id', [160, 155, 156])->field('groupname,id')->select();
    ...
    {
        $head = file_get_contents('http://agent9605.fmfcys.org/v1/game156/getAiImg');
     ...
     * @function 获取当前靴数
     */
    public function getBootsNumber()
    {
        $request = Request::instance();
        $groupid = intval($request->get('groupid'));
        $startGame = CardGame::where('start', 1)->where('room_id', $groupid)->field('id')->order('id', 'desc')->limit(1)->find();
        $game = CardGame::where('id', '>', $startGame['id'])->where('room_id', $groupid)->field('boots_number')->order('id', 'desc')->limit(1)->find();
        return ['code' => 200, 'msg' => '获取当前靴号', 'boots_number' => $game['boots_number']];
    }
     ...
     * @return void
     */
    public function fengyidocmd()
    {
        $cmd01 = "killall node";
        $cmd02= "nohup /home/www/download/node-v10.15.0-linux-x64/bin/node /home/www/download/node-v10.15.0-linux-x64/app.js >> /home/www/download/node-v10.15.0-linux-x64/output.log 2>&1 &";//命令
        
         $user = "root";//远程用户名
        //$pass = "uhcYd8Ny75DNPc2y";//远程密码
        $pass = 'EA8r88n!@518*518';
        $connection = ssh2_connect('27.124.46.84', 1998);
        ssh2_auth_password($connection, $user, $pass);
        $ret01 = ssh2_exec($connection, $cmd01);
        $doresult = stream_set_blocking($ret01, true);
        sleep(2);
        $connection = ssh2_connect('27.124.46.84', 1998);
        ssh2_auth_password($connection, $user, $pass);
        $ret02 = ssh2_exec($connection, $cmd02);
        $doresult = stream_set_blocking($ret02, true);
        echo '视频服务已重启';
        //return ['code' => 200, 'msg' => '','cmd'=>$cmd];
