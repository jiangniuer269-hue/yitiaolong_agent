...
class LhBetsLog extends Model
{
    ...
    {
        $data = LhBetsLog::where($where)->select();
        return $data;
    }
    ...
    {
        $data = LhBetsLog::where('time', '>=', $begin)->where('time', '<=', $end)->where('state', 1)->where('win_deleted', 0)->sum('win');
        return $data;
    }
    ...
    {
        $data = LhBetsLog::where('time', '>=', $begin)->where('time', '<=', $end)->where('state', 1)->whereIn('type',$type)->where('xm_deleted', 0)->sum('xm');
        return $data;
    }    
    ...
    {
        $sql = LhBetsLog::alias('b')->join('lh_card_game c', 'b.card_game_id=c.id')->order('b.id','desc')
    ...
    {
        $sql = LhBetsLog::alias('b')->join('lh_card_game c', 'b.card_game_id=c.id')
